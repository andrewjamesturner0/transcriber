 ...
 */
function mergeTranscriptWithDiarization(transcript, diarizeSegments, configOverrides) {
  ...
  const allWords = Array.isArray(transcript.words) ? transcript.words.map((word) => ({
    text: word.text,
    startMs: word.startMs != null ? word.startMs : word.start != null ? word.start * 1000 : null,
    endMs: word.endMs != null ? word.endMs : word.end != null ? word.end * 1000 : null,
  })) : [];

  if (allWords.length === 0 && Array.isArray(transcript.segments)) {
    for (const segment of transcript.segments) {
      if (!segment.text || !segment.text.trim()) continue;
      allWords.push({
        text: segment.text.trim(),
        startMs: segment.startMs != null ? segment.startMs : segment.start != null ? segment.start * 1000 : null,
        endMs: segment.endMs != null ? segment.endMs : segment.end != null ? segment.end * 1000 : null,
        isSegLevel: true,
      });
    }
  }

  for (const seg of (allWords.length === 0 && transcript.transcription || [])) {
    let words = null;
