...
function _buildWhisperArgs(modelPath, wavPath, jsonPrefix, options, modelSpec, isDtwSupportedFn) {
  ...
  const reduceRepeatedText = opts.reduceRepeatedText != null
    ? !!opts.reduceRepeatedText
    : !!opts.antiCorruption;
  if (reduceRepeatedText) {
    args.push('-mc', '0', '--temperature', '0.4', '--entropy-thold', '1.8');
  }

  if (opts.jobMode === 'translate') args.push('--translate');
  if (opts.sourceLanguage && !['auto', 'multilingual'].includes(opts.sourceLanguage)) {
    args.push('--language', opts.sourceLanguage);
  }
...
function createWhisperRunner({ capabilities, paths, spawn, log, tmpDir }) {
   ...
   * @param {object}       params.options     - { diarization, reduceRepeatedText, antiCorruption, jobMode, sourceLanguage, threads? }
